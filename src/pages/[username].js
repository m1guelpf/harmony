...
const Profile = ({ profile }) => {
	...
	const { data: artists } = useSWR(() => `/api/spotify/top?type=artists&timeRange=${artistPeriod}&username=${profile.username}`, fetcher)
	const { data: songs } = useSWR(() => `/api/spotify/top?type=tracks&timeRange=${songPeriod}&username=${profile.username}`, fetcher)
	...
	return (
		<div>
			...
			{session && profile.username !== session.user.username && (
				<span className="mt-4 w-full inline-flex rounded-md shadow-sm">
